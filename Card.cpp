...
Card::Card(const Card & card) {
    this->cardTypes = *new vector<string>(card.cardTypes);
    this->choice = new string(*(card.choice));
}

void Card::initialType() 
{}

vector<string> *Card::getpointer() {
    return &cardTypes;
}
...
Deck::Deck(const Deck &deck) {
    ...
    this->deck = *new vector<Card*> (deck.deck);
...
Hand::Hand(const Hand &hand) {
    ...
    this->playingCards = *new vector<Card*>(hand.playingCards);
}
...
void Hand::printHand() {
    if (handCards.size() > 0) {
        ...
        for (int i = 0; i < handCards.size(); i++) {
            cout << "  # " << i << " is " << *handCards.at(i)->getCardType() << endl;
        }
    } else {
...
vector<Card*>* Hand::getPlayingCard() {
    return &playingCards;
}
